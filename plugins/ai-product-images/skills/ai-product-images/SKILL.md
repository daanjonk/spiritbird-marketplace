...
**The `write_files` scope is critical** — it's required for uploading product photos and AI-generated images to Shopify Files. Without it, the staged upload flow will fail with "Access denied".

This stores credentials securely — no custom apps, API keys, or config files needed. Ask the user for their store domain at the start of the flow if not already known. The Shopify CLI handles all authentication for both store operations AND file uploads (including the staged upload to Google Cloud Storage — no separate Google login needed).
...
When the user drops an image in the chat, points to a file in their Downloads folder, or provides any local file path — use this flow to upload it to Shopify Files and get a permanent CDN URL.

**Auth requirement:** The user must have run `shopify store auth` with `write_files` scope. The staged upload uses pre-signed URLs generated by Shopify — no separate Google or cloud storage login is needed. Everything is handled through the Shopify CLI auth.

**If you get "Access denied for stagedUploadsCreate":** The user's current auth is missing the `write_files` scope. Re-run: `shopify store auth --store <store>.myshopify.com --scopes read_products,write_products,write_files`

This is a 4-step process that takes ~10 seconds total:

**Step 1: Get file info and create a staged upload target**

First, determine the file's size and MIME type:
```bash
# Get file size in bytes (macOS)
stat -f%z "<local_file_path>"
# On Linux: stat -c%s "<local_file_path>"

# Determine MIME type from file extension:
# .webp → image/webp | .jpg/.jpeg → image/jpeg | .png → image/png | .gif → image/gif
```

Then create the staged upload target:
```bash
...
- `url` — the Google Cloud Storage URL to upload to (e.g., `https://shopify-staged-uploads.storage.googleapis.com/`)
- `resourceUrl` — save this, you'll need it in Step 3
- `parameters` — an array of name/value pairs that authenticate the upload (pre-signed by Shopify)

**Step 2: Upload the file to the staged URL using curl**

Build a `curl` command using the `url` and every parameter from the `parameters` array. **Every single parameter must be included exactly as returned** — they contain the pre-signed authentication. The `file` field MUST be the last `-F` flag.

```bash
curl -s -o /dev/null -w "%{http_code}" -X POST "<url_from_step1>" \
  -F '<param1_name>=<param1_value>' \
  -F '<param2_name>=<param2_value>' \
  -F '<param3_name>=<param3_value>' \
  ... (include ALL parameters from the array) \
  -F "file=@<local_file_path>"
```

**Concrete example** (parameter names will match this pattern):
```bash
curl -s -o /dev/null -w "%{http_code}" -X POST "https://shopify-staged-uploads.storage.googleapis.com/" \
  -F 'Content-Type=image/webp' \
  -F 'success_action_status=201' \
  -F 'acl=private' \
  -F 'key=tmp/XXXX/files/UUID/filename.webp' \
  -F 'x-goog-date=20260416T073418Z' \
  -F 'x-goog-credential=merchant-assets@shopify-tiers.iam.gserviceaccount.com/...' \
  -F 'x-goog-algorithm=GOOG4-RSA-SHA256' \
  -F 'x-goog-signature=978c10b32aa768...' \
  -F 'policy=eyJjb25kaXRpb25z...' \
  -F "file=@/Users/username/Downloads/product-photo.webp"
```

**Expected response:** HTTP status code `201`. If you get `403`, the pre-signed URL may have expired — re-run Step 1. If you get `400`, check that all parameters are included and the file path is correct.

**Step 3: Create the file in Shopify using the resourceUrl from Step 1**
...
The response returns the file's `id` (format: `gid://shopify/MediaImage/XXXX`). The `fileStatus` will initially be `UPLOADED` — Shopify is processing it.

**Step 4: Poll for the permanent CDN URL**
...
- If `fileStatus` is `READY` and `image.url` is present → you have your permanent CDN URL
- If `fileStatus` is `UPLOADED` or `PROCESSING` → wait 3 more seconds and poll again
- Max 5 polls (~20 seconds total). If still not ready, something went wrong with the file.

**Result:** A permanent `https://cdn.shopify.com/...` URL ready for Kie AI. This URL never expires.

---

**Route B (simplified) — User provides a public URL:**

If the user provides an image that's already publicly accessible (e.g., from a website or existing CDN), skip the staged upload and use `fileCreate` directly with `originalSource`:
...
Then poll for the CDN URL as in Step 4 above. Shopify will fetch the image from the public URL and host it on its own CDN.
